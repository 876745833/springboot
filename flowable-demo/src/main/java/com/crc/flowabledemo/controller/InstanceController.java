package com.crc.flowabledemo.controller;


import com.crc.flowabledemo.common.result.ResponseData;
import com.crc.flowabledemo.common.utils.BeanUtils;
import com.crc.flowabledemo.flowable.handler.InstanceHandler;
import com.crc.flowabledemo.flowable.handler.TaskQueryHandler;
import com.crc.flowabledemo.repository.model.TaskVO;
...
public class InstanceController {
    ...
    @ResponseBody
    @ApiOperation(value = "启动流程实例__流程实例key和流程tenantId", notes = "实例启动成功，返回当前活动任务，如果部署流程模板时指定了tenantId，那么调用此方法也要指定", produces = "application/json")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "key", value = "流程定义key", required = true, dataType = "String"),
            @ApiImplicitParam(name = "tenantId", value = "系统标识", required = false, dataType = "String"),
    })
    public ResponseData startByName(String key, String tenantId, @RequestBody Map<String, Object> variables) {
        ProcessInstance pi = instanceHandler.startProcessInstanceByKeyAndTenantId(key, tenantId, variables);
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "key", value = "流程定义KEY（模板ID）", required = true, dataType = "String"),
            @ApiImplicitParam(name = "tenantId", value = "系统标识", required = false, dataType = "String"),
            @ApiImplicitParam(name = "userId", value = "流程启动者ID", required = true, dataType = "String"),
    })
    public ResponseData startAndExecuteAndSetActor(String key, String tenantId, String userId,
                                                   @RequestBody Map<String, Object> variables) {
        Map<String, Object> map = instanceHandler.startInstanceAndExecuteFirstTask(key, tenantId, userId, variables);
        return ResponseData.success(map);
    }
